...
namespace blockchain
{
    namespace storage
    {
        CStorageLocal::CStorageLocal()
        {
            ...
            {
                int status = mkdir(mBasePath.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH); // create directory
            }
        ...
        void CStorageLocal::loadChain(std::vector<CBlock*>* chain)
        {
            if(mMetaData.count("LAST_BLOCK_HASH") != 0)
            {
                ...
                if(chain->size() != chainSize)
                    throw std::runtime_error("Manifest: Chain size doesn't match BLOCK COUNT");
            }
        }
        ...
        void CStorageLocal::load(CBlock* block)
        {
            ...
            if(file)
            {
                ...
                if(r != 1)
                    throw std::runtime_error("Couldn't read version");
                ...
                if(r != 1)
                    throw std::runtime_error("Couldn't read createdTS");
                ...
                if(r != 1)
                    throw std::runtime_error("Couldn't read nonce");
                ...
                if(r != 1)
                    throw std::runtime_error("Couldn't read dataSize");
                ...
                for(uint32_t n = 0; n < dataSize; n += mChunkSize)
                {
                    ...
                    if(r == 0)
                        throw std::runtime_error("Couldn't read data chunk");
                    ptr += r;
                }
